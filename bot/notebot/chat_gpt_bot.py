...
class ChatGPTBot(Bot, OpenAIImage):
    def __init__(self):
        ...
        # model为notebot
        self.sessions = SessionManager(ChatGPTSession, model=conf().get("open_ai_api_key") or "qwen2-instruct")
        self.args = {
            "model": conf().get("open_ai_api_key") or "qwen2-instruct",  # 对话模型的名称
...
class AzureChatGPTBot(ChatGPTBot):
    ...
    def create_img(self, query, retry_count=0, api_key=None):
        ...
        if text_to_image_model == "dall-e-2":
            ...
            if not endpoint.endswith("/"):
                endpoint = endpoint + "/"
            url = "{}openai/deployments/{}/images/generations?api-version={}".format(endpoint, conf().get(
                "azure_openai_dalle_deployment_id", "text_to_image"), api_version)
            api_key = conf().get("azure_openai_dalle_api_key", "open_ai_api_key")
            headers = {"api-key": api_key, "Content-Type": "application/json"}
            try:
                body = {"prompt": query, "size": conf().get("image_create_size", "1024x1024"),
